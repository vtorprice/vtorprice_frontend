...
export const CompanyDocumentsFormManagement: React.FC<IWithClass> = ({
  className,
}) => {
...
export const CompanyNotRequiredDocumentsFormManagement: React.FC<IWithClass> = ({
  className,
}) => {
  ...
  return (
    <form className={className}>
      ...
      <div className="flex">
        <File className="w-full" name="Добавить другие документы" onSelect={(file) => { file ? console.error('---Ошибка: Неправильный формат файла') : null;return(file ? uploadDocumentEvent(file) : null)}} />
      </div>
